        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <label class="modal-title text-text-bold-600" id="title">التحاليل  </label>
...
<script type="text/javascript">
    $(function() {
        ...
        var table = $('#tableData').DataTable({
            ...
            ajax: "{{ route('classifications.index')}}",
...
$("#title").html("تعديل ");
$.get("{{ route('classifications.index') }}" + '/' + product_id + '/edit', function(data) {
                   $("#saveBtn").val("حفظ");
                   $('#_id').val(data.id);
                   $('#name').val(data.name);
                       $("#inlineForm").modal('show');
           ...
           $("#saveBtn").click(function (e) {
                ...
                var card_id = $("#_id").val();
                var method="post";
                var url = "{{ route('classifications.store') }}";
                if(card_id){
                   url = "{{ route('classifications.index') }}"+ '/' + card_id 
                   method="PATCH";}
                $.ajax({
        ...
        $('body').on('click', '.delete', function() {
            ...
            sweetConfirm(function(confirmed) {
                if (confirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('classifications.index') }}" + '/' + product_id,
                        data: {
                            '_token': '{{csrf_token()}}'
                        },
                        success: function(data) {
