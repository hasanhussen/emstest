        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <label class="modal-title text-text-bold-600" id="title">نوع الشركة  </label>
...
var uploadedDocumentMap = {}
    var alesDropZone  =   new Dropzone( "#document-dropzone", {
        url: '{{ route('projects.storeMedia',["table"=>"company_types"]) }}',
        maxFiles:1,
        maxFilesize: 2, // MB
        addRemoveLinks: true,
         headers: {
    ...
    <script type="text/javascript">
        ...
        $("#addClick").click(function (e) {
            ...
            $('body').on('click', '.edit', function () {
                ...
                $("#title").html("تعديل ");
                $.get("{{ route('company_type.index') }}" + '/' + product_id + '/edit', function (data) {
                    $("#saveBtn").val("حفظ");
                    $('#_id').val(data.id);
                    $('#type').val(data.type);
            ...
            $("#saveBtn").click(function (e) {
                ...
                var card_id = $("#_id").val();
                var method="post";
                var url = "{{ route('company_type.store') }}";
                if(card_id){
                   url = "{{ route('company_type.index') }}"+ '/' + card_id 
                   method="PATCH";}
                $.ajax({
            ...
            $('body').on('click', '.delete', function () {
                ...
                if (confirmed) {
                    $.ajax({
            type: "DELETE",
            url: "{{ route('company_type.index') }}"+ '/' + product_id,
            data:{
                '_token':'{{csrf_token()}}'
            },
            success: function (data) {
